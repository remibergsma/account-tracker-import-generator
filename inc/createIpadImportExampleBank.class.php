...
// class to process ExampleBank csv file and convert it to format readable by Account Tracker ipad app
class createIpadImportExampleBank extends createIpadImportCore {
	...
	protected function generateExportData() {
		...
		foreach($this->imported_data as $line) {
			...
			if($data[0]) {
				...
				$export_data['account'] = $this->matchAccount($data[1]); // field with your account number
				...
				$export_data['category'] = $this->matchCategory($data[4] . " " . $data[7]); // fields with description, company name etc. Used to match categories.
				$export_data['notes'] = "Note: " . $data[7] ."\nName: " . $data[4] ."\nAccount: " . $data[5] . "\nCode: " . $data[6] . "\nMy account: " . $data[1]; // All other fields that you want to appear in the notes.
				$export_data['chequenr'] = ''; // you can leave this empty but it needs to be at least an empty row
				$export_data['amount'] = $data[2]; // the amount of money: 123,45 with no separators for 1000's: 1234,56
